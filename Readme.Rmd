...
round(coef(fitlm2),2)
```
The amt coefficient is not that much changed and it is still insignificant with $95\%$ confidence interval of ($`r round(confint(fitlm2)[8,],2)`$). Let us take a look at the VIF's of the new model.
```{r echo=F, message=F}
round(vif(fitlm2),2)
...
```{r echo=F, message=F}
mtcars3 <- mtcars2[, -(maxvif2+1)]
fitlm3 <- lm(mpg ~ ., data = mtcars3)
round(coef(fitlm3),2)
```
Again a small decrease in the am coefficient and still non-significant (confidence interbal: $`r round(confint(fitlm3)[7,],2)`$). What about VIF'f of this thord model.
```{r echo=F, message=F}
round(vif(fitlm3),2)
maxvif3 <- which(vif(fitlm3)==max(vif(fitlm3)))
```


The VIF's of this last model are in a quite similar range. So there is no sense in ommiting any of them. Now for assurance we comare the first and last models with anova function.
```{r echo=F, message=F}
anova(fitlm3, fitlm1)
```
The P-value is quite high, so ommiting these two varaibales was not actually necessary. 

To get sure abput the validity of the last model I plot the residual plot.

```{r echo=F, message=F}
plot(predict(fitlm3),resid(fitlm3), xlab = "Fitted values", ylab = "Residuals")
abline(h=0, col="red")
```
As you can see there is no special trend in the residual plot. So the model appears to be valid. 
