...
```
We have 10 variables to include in the regression model. But clearly some of them are higky correlated which make unvanted variance in the model (for instance number of cylinders and horse power). I computed the corrolation between varoable and ommited those which are more than $80\%$ corrolated. Which ommit `r names(mtcars)[highlyCorrelated]` variables and leave me with `r names(mtcars)[-highlyCorrelated]`.
